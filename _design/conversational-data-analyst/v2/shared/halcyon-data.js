...
(function (root) {
  ...
  /* ---- Q4: retail vs SME head-to-head (FY2025) ----
   * Reuses the onboarding series verbatim so the two answers can never disagree.
   */
  var retailVsSme = {
    months: MONTHS,
    series: [onboarding.series[0], onboarding.series[1]],
    totals:   { retail: 6575, sme: 1974 },
    ratio:    3.33,
    growth:   { retail: 63.6, sme: 81.4 },
    approval: { retail: 84.1, sme: 78.6 },
    avgDays:  { retail: 2.4,  sme: 6.1  }
  };
  ...
  var runs = {
    ...
    branches: {
      ...
      intent: 'rank - ratio - join',
      tables: ['onboarding_applications', 'branches'],
      columns: ['branch_id', 'decision', 'branch_name'],
      sql: [
        'SELECT',
        '  b.branch_name,',
        '  COUNT(*)                                 AS applications,',
        '  COUNT(*) FILTER (WHERE a.decision = $1)  AS rejected,',
        '  ROUND(100.0 * COUNT(*) FILTER (WHERE a.decision = $1)',
        '        / NULLIF(COUNT(*), 0), 1)          AS rejection_rate',
        'FROM onboarding_applications a',
        '  JOIN branches b ON b.branch_id = a.branch_id',
        "WHERE a.submitted_at >= DATE '2025-01-01'",
      ...
      checks: [
        { label: 'Single statement',     status: 'pass', detail: 'One statement' },
        { label: 'Read-only (SELECT)',   status: 'pass', detail: 'No write verbs present' },
        { label: 'Tables on allow-list', status: 'pass', detail: 'onboarding_applications, branches' },
        { label: 'Row limit enforced',   status: 'pass', detail: 'LIMIT 20 appended by planner' },
        { label: 'Parameters bound',     status: 'pass', detail: "$1 = 'rejected' (bound, not interpolated)" }
      ],
      plan: 'Hash join + filtered aggregate - cost 1,884.2',
      rows: 6,
      ms: 112
    },
    ...
    compare: {
      ...
      intent: 'compare - two cohorts - time series',
      tables: ['onboarding_applications'],
      columns: ['submitted_at', 'segment', 'decision'],
      sql: [
        'SELECT',
        "  date_trunc('month', submitted_at) AS month,",
        '  segment,',
        '  COUNT(*)                                     AS applications,',
        '  ROUND(100.0 * COUNT(*) FILTER (WHERE decision = $1)',
        '        / NULLIF(COUNT(*), 0), 1)              AS approval_rate',
        'FROM onboarding_applications',
        'WHERE segment IN ($2, $3)',
        "  AND submitted_at >= DATE '2025-01-01'",
        'GROUP BY 1, 2',
        'ORDER BY 1, 2',
        'LIMIT 1000;'
      ].join('\n'),
      checks: [
        ...
        { label: 'Tables on allow-list', status: 'pass', detail: 'onboarding_applications' },
        { label: 'Row limit enforced',   status: 'pass', detail: 'LIMIT 1000 appended by planner' },
        { label: 'Parameters bound',     status: 'pass', detail: "3 bound: 'approved', 'Retail', 'SME'" }
      ],
      plan: 'Seq scan + hash aggregate - cost 596.4',
      rows: 24,
      ms: 88
    }
  };
